...
// забираем в скрипт установленные переменные из файлов

    
    if ($otladka) {
    echo '<p><b>var_dump(file_exists("ads.txt"))= ';
    echo var_dump(file_exists('ads.txt')).'</b></p>';
    
        /* для отладки ошибки 2
    $_POST = array ( 'private' => '1' , 'email' => '' , 'title' => '2', 'price' => '200', 'seller_name' => 'Алина', 'main_form' => 'Добавить' );
    $ads_h=fopen($ads_f,'r+');
    ...
    if ($otladka) {
    echo '<p><b>Начальная установка var_dump($ads_h)= </b></p>';
    var_dump($ads_h); 
    }
*/
...
if (file_exists('ads.txt')) {
    ...
    if ($otladka) {
    echo '<p><b>Файл существует и текст там = var_dump($ads_t)= </b></p>';
    var_dump($ads_t);
    
        echo '<p><b>Файл существует и var_dump($temp_array)= </b></p>';
    var_dump($temp_array);
...
if ($_POST['main_form']=='Добавить') {
        ...
        if ($otladka) { 
                                echo "<p>Пост -Добавить -1 -  temp_array= </p>\n";
        var_dump($temp_array);
                }
        ...
        array_push($temp_array,$_POST);
                ...
                if ($otladka) { 
                                echo "<p>Пост -Добавить -2 -  temp_array= </p>\n";
        var_dump($temp_array);
                }
